...
class AudiosetDataset(Dataset):
    ...
    def __init__(self, dataset_json_file, audio_conf, label_csv=None, mixup_factor=[0.3, 0.7], training=False):
        ...
        if self.training:
            print("########training dataset: ", len(self.data))
        else:
            print("########val dataset: ", len(self.data))
    ...
    def _wav2fbank(self, filename, filename2=None):
        ...
        # mixup
            ...
            mix_lambda = np.random.uniform(self.mixup_factor[0], self.mixup_factor[1])
    ...
    def __getitem__(self, index):
        ...
        # #     fbank = freqm(fbank)
        if self.timem != 0:
            fbank = timem(fbank[None, :, :])[0]
        fbank = torch.transpose(fbank, 0, 1)

        # self.t_m.append(fbank.mean())
        # self.t_std.append(fbank.std())
        # # if len(self.t_m) % 100==0:
        # #     print("!!!!!!!!!!!!!mean:", np.mean(self.t_m))
        # #     print("!!!!!!!!!!!!!std:", np.mean(self.t_std))
