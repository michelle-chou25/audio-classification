...
class MBNet(nn.Module):
    def __init__(self, label_dim=527, pretrain=True):
        ...
        self.model.conv1[0] = torch.nn.Conv2d(2, 24//f, kernel_size=(3, 3), stride=(2, 2), padding=(1, 1), bias=False)
        ...
        #     att_activation='sigmoid',
        #     cla_activation='sigmoid')
        logger.info( 'Total parameter number is : {:.3f} million'.format(sum(p.numel() for p in self.model.parameters()) / 1e6))
