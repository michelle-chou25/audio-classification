...
audio_model = torch.jit.load('./mbv3_cpu.pth')
...
for idx in range(999):
    data_mk = stream.read(int(RATE * interval))
    start_ = time.time()
    out_data = np.fromstring(data_mk, dtype=np.int16)[None, :]
    # out_data = np.fromstring(data_mk, dtype=np.float32)[None, :]
    out_data = torch.from_numpy(out_data/32768)
    # out_data = torch.from_numpy(out_data)
    data = r.preprocess(out_data, RATE)
    ...
    if ret.argmax() == 0:
        ...
        cv2.imshow('result:', no)
    cv2.waitKey(1)
    save_name = os.path.join("../test_ret/OK",
    ...
    # librosa.output.write_wav(save_name, out_data[0].numpy().astype(np.float32), ori_sr)
    if ret[0][1] < 0.7:
        soundfile.write(save_name, out_data[0].numpy().astype(np.float32), RATE)
