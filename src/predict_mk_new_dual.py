...
audio_model = torch.jit.load('./7.20_ruo_error_env_error_mbv3_cpu.pth')
...
for idx in range(999):
    data_mk1 = stream1.read(int(RATE * interval))
    data_mk2 = stream2.read(int(RATE * interval))
    start_ = time.time()
    ...
    if ret.argmax() == 0:
        ...
        cv2.imshow('result:', no)
    cv2.waitKey(1)
    # save_name = os.path.join("../test_ret/ruo3_xiawu",
