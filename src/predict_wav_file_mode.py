...
if args.model == 'efficientnet':
    ...
    audio_model = models.MBNet(label_dim=args.n_class, pretrain=False)
audio_model.cuda()
...
# state_dictBA = torch.load("../exp/3.14_93.6/models/best_audio_model.pth", map_location='cpu')
...
save_cpu_model.save('mbv3_cpu.pth')

save_gpu_model = torch.jit.script(audio_model.cpu())
save_gpu_model.save('mbv3_gpu.pth')
exit(0)
...
# ori_data, ori_sr = torchaudio.load(r'/home/share_data/ext/PVDefectData/test2021/zh/dt/wz/__org__/TestData/q+r_1.WAV')
...
ori_data = torchaudio.transforms.Resample(ori_sr, 22050)(ori_data[0, :])
...
for idx, i in enumerate(range(0, len(ori_data), int(1.5 * ori_sr))):
# for idx, i in enumerate(range(0, len(ori_data[0]), int(0.25 * ori_sr))):
    ...
    if i + interval_rate < len(ori_data):
        ...
        data = r.preprocess(out_data[None, :], ori_sr)
        ...
        sf.write(save_name, out_data.numpy().astype(np.float32), ori_sr, subtype='PCM_24')
